...
typedef struct {
    ...
    GLint mvp_loc, tex_loc, pos_loc, uv_loc;
    World world;
...
static GLuint load_tex(AAssetManager* a,const char* name) {
    ...
    if(!f) { LOGI("Texture not found: %s", name); return 0; }
    ...
    free(d); if(!img) { LOGI("Failed to decode: %s", name); return 0; }
...
static void game_init(Game* g, AAssetManager* assets) {
    ...
    g->mvp_loc=glGetUniformLocation(g->prog,"uMVP");
    g->tex_loc=glGetUniformLocation(g->prog,"uTex");
    g->pos_loc=glGetAttribLocation(g->prog,"aPos");
    g->uv_loc=glGetAttribLocation(g->prog,"aUV");
...
static void game_update(Game* g) {
    g->velocity=v3_add(g->velocity,v3(0,0,-0.008f));
    if(g->fwd) { float a=g->yaw*3.14159f/180.0f; Vec3 d=v3(cosf(a),sinf(a),0); g->velocity=v3_add(g->velocity,v3_scale(d,g->fwd*0.05f)); }
    if(g->side) { float a=g->yaw*3.14159f/180.0f; Vec3 d=v3(cosf(a-1.57f),sinf(a-1.57f),0); g->velocity=v3_add(g->velocity,v3_scale(d,g->side*0.05f)); }
    if(g->jump && g->on_ground) g->velocity.z=0.15f;
    
    Vec3 np=v3_add(g->position,g->velocity);
    Block* b=world_get(&g->world,(int)np.x,(int)g->position.y,(int)g->position.z);
    if(b && b->type!=AIR) { g->velocity.x=0; np.x=g->position.x; }
    b=world_get(&g->world,(int)g->position.x,(int)np.y,(int)g->position.z);
    if(b && b->type!=AIR) { g->velocity.y=0; np.y=g->position.y; }
    b=world_get(&g->world,(int)g->position.x,(int)g->position.y,(int)np.z);
    if(b && b->type!=AIR) { if(np.z<g->position.z) g->on_ground=1; g->velocity.z=0; np.z=g->position.z; } else g->on_ground=0;
    g->position=np;
    g->velocity=v3_scale(g->velocity,0.85f);
    
    if(g->dig || g->place) {
        float a=g->yaw*3.14159f/180.0f, p=g->pitch*3.14159f/180.0f;
        Vec3 dir=v3(cosf(p)*cosf(a), cosf(p)*sinf(a), sinf(p));
        Vec3 cam=v3_add(g->position,v3(0,0,1.6f));
...
static void game_render(Game* g) {
    ...
    glUseProgram(g->prog);
    glUniformMatrix4fv(g->mvp_loc,1,GL_FALSE,mvp.m);
    glUniform1i(g->tex_loc,0);
    
    // Куб из треугольников (36 вершин)
    float cube_verts[] = {
    ...
    for(int ci=0;ci<g->world.count;ci++) {
        ...
        for(int bz=0;bz<CHUNK_SIZE;bz++) {
            ...
            for(int i=0;i<36*5;i++) {
                ...
                int c = i%5;
                if(c<3) {
                    float val = cube_verts[i];
                    if(c==0) val += x;
                    else if(c==1) val += y;
                    else val += z;
                    verts[i] = val;
                } else {
                    verts[i] = cube_verts[i];
                }
            }
            glEnableVertexAttribArray(g->pos_loc);
            glEnableVertexAttribArray(g->uv_loc);
            glVertexAttribPointer(g->pos_loc,3,GL_FLOAT,GL_FALSE,5*sizeof(float),verts);
            glVertexAttribPointer(g->uv_loc,2,GL_FLOAT,GL_FALSE,5*sizeof(float),verts+3);
            glDrawArrays(GL_TRIANGLES,0,36);
        }
    }
    glDisableVertexAttribArray(g->pos_loc);
    glDisableVertexAttribArray(g->uv_loc);
}
